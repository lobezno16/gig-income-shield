...
async def feature_importance(request: Request, _admin: Worker = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    ...
    if model_path.exists():
        last_trained = datetime.fromtimestamp(model_path.stat().st_mtime, tz=timezone.utc).isoformat()
        ...
        bcr_rows = (
            await db.execute(
                ...
                .where(
                    ...
                    BCRRecord.period_end >= (datetime.now(timezone.utc) - timedelta(days=84)).date(),
                )
                .order_by(desc(BCRRecord.period_end))
                .limit(12)
            )
        ).scalars().all()
        bcr_rows = list(reversed(bcr_rows))
            ...
            "current": (
                {
                    ...
                    "last_updated": posterior.last_updated.isoformat() if posterior.last_updated else None,
                }
                if posterior
                else None
            ),
