...
<img src="https://img.shields.io/badge/🏛️-Soteria-black?style=for-the-badge&labelColor=0a0a0a&color=4F46E5" alt="Soteria" />
...
[![Guidewire DEVTrails 2026](https://img.shields.io/badge/Guidewire-DEVTrails%202026-0ea5e9?style=flat-square)](https://devtrails.guidewire.com)
[![Phase 2](https://img.shields.io/badge/Phase-2%20Submission-4F46E5?style=flat-square)]()
[![Persona](https://img.shields.io/badge/Persona-Q--Commerce%20Grocery-f59e0b?style=flat-square)]()
[![Team](https://img.shields.io/badge/Team-DevGodz-4F46E5?style=flat-square)]()
...
```mermaid
flowchart LR
    subgraph "Layer 1: Batch (Weekly)"
        HD["10-Year\nHistorical Data"] --> FE["Feature\nEngineering"]
        FE --> RF["Random Forest\nn=200, depth=8"]
        RF --> SHAP_E["SHAP\nExplainer"]
    end

    subgraph "Layer 2: Online (Per Claim)"
        CE["Claim Event"] --> BU["Bayesian\nBeta Update"]
        BU --> TP["Updated Trigger\nProbability"]
    end

    RF --> ADJ["Premium\nAdjustment\n−₹5 to +₹5"]
    SHAP_E --> EXP["Per-Worker\nExplanation"]
    TP --> BASE["Base Formula\nTrigger Prob × Income\n× Days"]
    ADJ --> FINAL["Final Premium\n₹20-₹50"]
    BASE --> FINAL

    style RF fill:#4F46E5,color:#fff
    style BU fill:#4F46E5,color:#fff
    style FINAL fill:#22c55e,color:#fff
    style SHAP_E fill:#7c3aed,color:#fff
```
...
```mermaid
flowchart TD
    A["📊 Calculate BCR\nTotal Claims ÷ Total Premiums"] --> B{"BCR in range?"}
    B -->|"0.55–0.70 ✅"| C["Healthy Pool\nContinue enrolments"]
    B -->|"0.70–0.85 ⚠️"| D["Warning Zone\nReview pricing\nAdjust tier factors"]
    B -->|"> 0.85 🛑"| E["SUSPEND\nNew enrolments\nfor this zone/peril"]
...
```mermaid
sequenceDiagram
    participant API as 🌧️ CPCB/IMD API
    participant K as 📡 SENTINELLE
    participant DB as 🗄️ PostgreSQL + H3
    participant F as 🔍 ARGUS Fraud Engine
    participant P as 💳 Razorpay (UPI)
    participant GW as 🏢 Guidewire Mocks
    participant W as 👤 Ravi (Worker)

    API->>K: AQI 380 detected — Delhi NCR station
    K->>DB: H3 hex lookup — find active policies in affected hexes
    DB-->>K: 234 workers matched via H3 spatial index
    K->>F: Validate Ravi — 4-layer pipeline (rules → 7-signal → IF → DBSCAN)
    F->>F: Layer 0: GPS ✓ Login ✓ Warranty ✓ No dup ✓
    F->>F: Layer 1: Trust Score = 0.91 (cell tower ✓ accelerometer ✓ battery ✓)
    F->>F: Layer 2: Isolation Forest anomaly = 0.12 (normal)
    F->>F: Layer 3: Regional Z-Score = 0.3 (consistent with hex)
    F-->>K: APPROVED — combined fraud score = 0.18
    K->>P: Initiate ₹500 payout → Ravi's UPI
    P-->>K: Payment confirmed ✅
    K->>GW: PolicyCenter.log_payout() + BillingCenter.reconcile()
    K->>W: SMS + haptic + voice: "₹500 transferred to your UPI"
    K->>DB: Update BCR + Bayesian posterior for this hex
```
...
```mermaid
flowchart TD
    A["🚨 150 claims in 8 minutes\nChennai Zone 4"] --> B["🔍 Ring Detection\n(ARGUS Layer 3)"]

    B --> C["⏱️ TEMPORAL\nLegit: trickle over 30-60 min\n150 in 8 min = anomaly"]
    B --> D["📍 GPS QUANTIZATION\nSpoofing: exact decimals\nReal storm: fluctuating GPS"]
    B --> E["👥 SOCIAL GRAPH\nContact overlap?\nBatch-registered?\nShared UPI accounts?"]
    B --> F["📱 PLATFORM CHECK\nZepto says OFFLINE\nbefore the alert"]

    C & D & E & F --> G{"🔗 Ring Confidence\n> 0.70?"}
    G -->|"YES"| H["🛑 FREEZE cluster\nLegit individuals triaged\nand paid within 4 hours"]
    G -->|"NO"| I["Process each claim\nthrough Trust Score Engine"]
```
...
```mermaid
flowchart TD
    TE["🌧️ Trigger Event"] --> L0
    
    subgraph L0["Layer 0: Rule-Based Pre-Checks"]
        GPS["GPS in H3 hex? ✓/✗"]
        LOGIN["Platform login active? ✓/✗"]
        WARRANTY["7+ active days? ✓/✗"]
        DUP["No duplicate claim? ✓/✗"]
        HOURS["Active during trigger? ✓/✗"]
    end
    
    L0 -->|"All pass"| L1
    L0 -->|"Any fail"| BLOCK["🚫 Payout Blocked"]
    
    subgraph L1["Layer 1: 7-Signal Trust Score"]
        SIG["Cell Tower · GPS Accuracy\nMotion · Wi-Fi · Battery\nNetwork · Platform Status"]
        SIG --> TS["Trust Score\n0 to 1"]
    end
    
    L1 -->|"Score ≥ 0.80"| FAST["⚡ Fast-Track Approve"]
    L1 -->|"Score < 0.50"| FLAG["⚠️ Flagged for Review"]
    L1 -->|"Score 0.50–0.80"| L2
    
    subgraph L2["Layer 2: Isolation Forest"]
        FEAT["6 Features:\nclaim_freq, avg_payout,\ngps_variance, login_duration,\ndays_since_reg, neighbor_ratio"]
        FEAT --> ISO["Isolation Forest\ncontamination=0.05"]
        ISO --> ASCORE["Anomaly Score"]
    end
    
    L2 -->|"Normal"| L3
    L2 -->|"Anomalous"| FLAG2["⚠️ Admin Review"]
    
    subgraph L3["Layer 3: DBSCAN + Regional Z-Score"]
        DBSCAN_N["DBSCAN Collusion\neps=0.02, min=3"]
        ZSCORE["Regional Z-Score\nworker vs hex avg"]
        DBSCAN_N --> COMBINED["Combined Score"]
        ZSCORE --> COMBINED
    end
    
    L3 -->|"Score < 0.8"| PAY["✅ Payout Approved"]
    L3 -->|"Score ≥ 0.8"| FLAG3["⚠️ Admin Review"]
    FAST --> PAY
    
    style L0 fill:#1e1b4b,color:#fff
    style L1 fill:#1e1b4b,color:#fff
    style L2 fill:#1e1b4b,color:#fff
    style L3 fill:#1e1b4b,color:#fff
    style PAY fill:#22c55e,color:#fff
...
```mermaid
flowchart TB
    subgraph S1["System 1: ATHÉNA Premium Intelligence"]
        RF["Random Forest + SHAP\nn=200, depth=8"] --> FI["Explainable\nFeature Importance"]
        BB["Bayesian Beta-Binomial\nConjugate Prior"] --> TUP["Real-Time Trigger\nProbability"]
    end

    subgraph S2["System 2: SENTINELLE Trigger Calibration"]
        MC["Monte Carlo\n10,000 iterations"] --> TP2["Per-Hex Trigger\nProbability"]
        ES["Holt-Winters\nExponential Smoothing"] --> FC["7-Day Peril\nForecast"]
    end

    subgraph S3["System 3: ARGUS Fraud Detection"]
        TS_S["7-Signal Trust Score\nEngine"] --> FS2["Trust Score\n0-1"]
        IF2["Isolation Forest\ncontamination=0.05"] --> AS2["Anomaly Score"]
        DBS2["DBSCAN\neps=0.02, min=3"] --> CR2["Ring Detection"]
        ZS2["Regional Z-Score"] --> FSC["Combined\nFraud Score"]
        FS2 --> FSC
        AS2 --> FSC
        CR2 --> FSC
    end

    subgraph S4["System 4: PYTHIA Actuarial Stress Engine"]
        MCS2["Monte Carlo\nStress Simulation"] --> CI2["90% Confidence\nInterval"]
        SA2["Sensitivity Analysis\nOAT Method"] --> PA2["Pool Adequacy\nReport"]
    end

    subgraph S5["System 5: Liquidity Shield (Prophet)"]
        PR["Facebook Prophet\nSeasonal Decomposition"] --> RF2["Reserve\nForecasting"]
        PR --> CL2["Claim Exposure\nPrediction"]
    end

    style S1 fill:#1e1b4b,color:#fff
    style S2 fill:#1e1b4b,color:#fff
    style S3 fill:#1e1b4b,color:#fff
    style S4 fill:#1e1b4b,color:#fff
    style S5 fill:#1e1b4b,color:#fff
```
...
flowchart TB
    ...
    subgraph EI["📡 Event Ingestion"]
        CPCB["CPCB/IMD APIs"] --> Poller["SENTINELLE\n(Celery + Redis\nevery 15 min)"]
        Poller --> TQ["Redis Trigger Queue"]
    end

    subgraph CP["⚙️ Core Processing"]
        TQ --> Proc["Trigger Processor"]
        Proc --> Fraud["ARGUS 4-Layer\nFraud Pipeline"]
        Fraud --> Calc["Payout Calculator"]
        Calc --> Settle["HERMÈS Settlement\n(UPI/IMPS + Rollback)"]
    end

    subgraph GW["🏢 Guidewire Mocks"]
        Settle --> PC["PolicyCenter"]
        Settle --> BC["BillingCenter"]
    end

    subgraph ML["🧠 ML Pipeline"]
        Hist[("10-yr Ward-Level\nHistorical Data")] --> RFT["Random Forest\nTrainer (weekly)"]
        RFT --> MS["Model Store"]
        BU2["Bayesian Online\nUpdater"] --> MS
        MS --> PE["ATHÉNA\nPremium Engine"]
        ProphetM["Prophet\nLiquidity Model"] --> Reserve["Reserve\nForecasting"]
    end
    ...
    subgraph PAY["💳 Payments"]
        Settle --> RZ["Razorpay Sandbox\nUPI + IMPS"]
    end

    Worker --> API["⚡ FastAPI Gateway"]
    Admin --> API
    API --> Proc
    API --> PE
    Proc --> DB
...
```mermaid
gantt
    title Soteria — 6-Week Execution Plan
    ...
    Backend + H3 + ATHÉNA ML Engine     :done, p2a, 2026-03-21, 7d
    7 Triggers + SENTINELLE             :done, p2b, 2026-03-28, 5d
    ARGUS 4-Layer Fraud Pipeline        :done, p2e, 2026-03-30, 3d
    HERMÈS Claims + Actuarial + PYTHIA  :active, p2c, 2026-04-01, 4d
    Frontend Registration + Dashboard   :active, p2d, 2026-04-02, 3d

    section Phase 3 — Scale & Optimize
    Razorpay Live + Capacitor Native    :p3b, 2026-04-05, 4d
    Prophet Liquidity + Polish          :p3c, 2026-04-09, 5d
    Final Testing + 5-min Demo          :p3d, 2026-04-14, 3d
```
